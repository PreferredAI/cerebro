...
public class HNSWScripts {
    ...
    static String idxfolder = "./hnsw-%dconn-%dseg";
    static String labelFile = "./data/trueTop20_itemsQuery.o";
    static String vectFile = "./data/user_vect.o";

    @Test
    public void testBuildHNSWIdx(){

        float[][] vecs = null;
        String indexDir = String.format(idxfolder, nconns, nseg);
        IndexUtils.ensureDirExist(indexDir);
        FloatDotHandler handler = new FloatDotHandler();
        vecs = handler.load(new File(vectFile))[0];
        ...
        System.out.println("Index construction time: " + (end - start) +"ms");
    }

    @Test
    public void testSearchHNSWIdx(){
        int k = 20;
        String idxDir = String.format(idxfolder, nconns, nseg);
        String queryLabelDir = labelFile;
